...
@st.cache_resource
def load_llm():
    """Load Phi-3-mini with fallback and logging"""
    cache_path = "./models"
    ...
    try:
        ...
        Try the following:
        - Reduce model size (Phi-3 Mini may be too large for CPU)
        - Upgrade your Streamlit plan (memory limit may be too small)
        - Run locally for dev and debugging
        """)
        st.stop()
        
@st.cache_resource
def load_vectorstore():
    """Load vectorstore with better fail-safety"""
    try:
        ...
        if collection_count == 0:
            logger.warning("⚠️ Empty vectorstore.")
            st.warning("⚠️ No documents found in vectorstore. Please upload or index them.")
        else:
            logger.info(f"Vectorstore loaded: {collection_count} docs")
        ...
        logger.error(traceback.format_exc())
        st.error("Vectorstore could not be loaded. Check if ChromaDB is properly configured.")
        st.stop()
