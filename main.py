...
try:
    from fpdf import FPDF  # fpdf2 (modern)
except ImportError:
    try:
        from fpdf2 import FPDF  # alternative import
    except ImportError:
...
@st.cache_resource
def load_llm():
    """Load Phi-3-mini with optimized configuration and robust error handling"""
    cache_path = "./models"
    ...
    try:
        ...
        **Troubleshooting Steps:**
        1. **Memory Issues**: Try restarting the app or reducing model size
        2. **Network Issues**: Check internet connection and retry
        3. **Cache Issues**: Delete `./models` folder and restart
        4. **GPU Issues**: Model will fallback to CPU (slower but functional)
...
# --- Enhanced Core QA Function ---
def ask_specbot(query):
    """Enhanced QA with better prompt engineering and error handling"""
    # Input validation and sanitization
    if not query or len(query.strip()) < 3:
        return "Please provide a more specific question.", ""
    
    sanitized_query = query.strip()[:800]  # Reasonable limit
    
    try:
        # Enhanced prompt template for Phi-3
        system_prompt = """You are an expert assistant for Ghana Highway Authority (GHA) specifications. 
        Provide accurate, specific answers based only on the provided context.
        If information is not in the context, clearly state this.
        Keep answers concise but complete."""
        
        # Format query for Phi-3
...
<|assistant|>"""
        ...
        retriever = vectorstore.as_retriever(
            ...
            search_kwargs={
                ...
                "fetch_k": 8,  # Fetch more candidates before MMR
        ...
        qa_chain = RetrievalQA.from_chain_type(
            ...
            chain_type_kwargs={
                "prompt": None,  # Let the model use its default prompt handling
